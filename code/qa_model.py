...
class QAModel(object):
    ...
    def __init__(self, FLAGS, id2word, word2id, emb_matrix):
        ...
        self.saver = tf.train.Saver(
            tf.global_variables(), max_to_keep=FLAGS.keep)
        self.bestmodel_saver = tf.train.Saver(
    ...
    def add_placeholders(self):
        ...
        self.context_mask = tf.placeholder(
            tf.int32, shape=[None, self.FLAGS.context_len])
        self.qn_ids = tf.placeholder(
            tf.int32, shape=[None, self.FLAGS.question_len])
        self.qn_mask = tf.placeholder(
            tf.int32, shape=[None, self.FLAGS.question_len])
    ...
    def add_embedding_layer(self, emb_matrix):
        ...
        with vs.variable_scope("embeddings"):
            ...
            self.context_embs = embedding_ops.embedding_lookup(
                embedding_matrix, self.context_ids)
            # shape (batch_size, question_len, embedding_size)
            self.qn_embs = embedding_ops.embedding_lookup(
    ...
    def build_graph(self):
        ...
        context_to_question, question_to_context = attn_layer.build_graph(
    ...
    def add_loss(self):
        ...
        with vs.variable_scope("loss"):
            ...
            loss_start = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=self.logits_start, labels=self.ans_span[:, 0])  # loss_start has shape (batch_size)
    ...
    def run_train_iter(self, session, batch, summary_writer):
        ...
        output_feed = [self.updates, self.summaries, self.loss,
                       self.global_step, self.param_norm, self.gradient_norm]
    ...
    def train(self, session, train_context_path, train_qn_path, train_ans_path, dev_qn_path, dev_context_path, dev_ans_path):
        ...
        summary_writer = tf.summary.FileWriter(
            self.FLAGS.train_dir, session.graph)
            ...
            for batch in get_batch_generator(self.word2id, train_context_path, train_qn_path, train_ans_path, self.FLAGS.batch_size, context_len=self.FLAGS.context_len, question_len=self.FLAGS.question_len, discard_long=True):
                ...
                if global_step % self.FLAGS.eval_every == 0:
                    ...
                    dev_loss = self.get_dev_loss(
                        session, dev_context_path, dev_qn_path, dev_ans_path)
                    logging.info("Epoch %d, Iter %d, dev loss: %f" %
                                 (epoch, global_step, dev_loss))
                    write_summary(dev_loss, "dev/loss",
                                  summary_writer, global_step)

                    # Get F1/EM on train set and log to tensorboard
                    train_f1, train_em = self.check_f1_em(
                        session, train_context_path, train_qn_path, train_ans_path, "train", num_samples=1000)
                    logging.info("Epoch %d, Iter %d, Train F1 score: %f, Train EM score: %f" % (
                        epoch, global_step, train_f1, train_em))
                    write_summary(train_f1, "train/F1",
                                  summary_writer, global_step)
                    write_summary(train_em, "train/EM",
                                  summary_writer, global_step)

                    # Get F1/EM on dev set and log to tensorboard
                    dev_f1, dev_em = self.check_f1_em(
                        session, dev_context_path, dev_qn_path, dev_ans_path, "dev", num_samples=0)
                    logging.info("Epoch %d, Iter %d, Dev F1 score: %f, Dev EM score: %f" % (
                        epoch, global_step, dev_f1, dev_em))
                    write_summary(dev_f1, "dev/F1",
                                  summary_writer, global_step)
                    write_summary(dev_em, "dev/EM",
                                  summary_writer, global_step)
                    ...
                    if best_dev_em is None or dev_em > best_dev_em:
                        ...
                        self.bestmodel_saver.save(
                            session, bestmodel_ckpt_path, global_step=global_step)
