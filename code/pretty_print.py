...
def print_example(word2id, context_tokens, qn_tokens, true_ans_start, true_ans_end, pred_ans_start, pred_ans_end, true_answer, pred_answer, f1, em):
    ...
    context_tokens = [w if w in word2id else "_%s_" %
    ...
    context_tokens[pred_ans_start] = magentaback(
        context_tokens[pred_ans_start])
    ...
    if truncated:
        ...
        print(
            redtext("{:>22}(True answer was truncated from context)".format("")))
    else:
